...
def main():
    # bad programming, to change a constant's value like this... I promise it was originally a constant :(
    for i in range(5,45,5):
        K = i
        no_of_cols = 1 + 8 + (1839*2) + (K*2) + (1*7)
        ...
        for f in os.listdir(sys.argv[1]):
            if f.endswith('.csv'):
                ...
                t_features = transfer_features(current_capture, simple)
...
def label_quic_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df.columns = ["time_frame_epoch", "src_ip", "dst_ip", "src_port", "dst_port", "src_port(TCP)", "dst_port(TCP)", "proto", "ip_hdr_len", "udp_len", "data_len", "time_delta", "time_relative", "udp_stream", "expert_msg", "seq_num"]
    return df
...
def label_tcp_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df.columns = ["time_frame_epoch", "src_ip", "dst_ip", "src_port_udp", "dst_port_udp", "src_port", "dst_port", "proto", "ip_len", "ip_hdr_len", "tcp_hdr_len", "notworking_data_len", "data_len", "tsval", "tsecr", "tcp_seq", "tcp_ack", "tcp_win_size", "expert_msg", "change_cipher_spec" "seq_num"]
    return df
...
# ADVANCED FEATURES:
def transfer_features(df: pd.DataFrame, simple_features):
    ...
    ordered_pkt_lengths = packet_order(df) # kd
    #print(f"ordered pkt lengths = {len(ordered_pkt_lengths)}dim")
    ita_time = interarrival_time(df) # kd
...
"""
def packet_order(df: pd.DataFrame):
    ...
    if len(packet_lengths_in_order) < K:
        return list(np.pad(np.array(packet_lengths_in_order), (0, 40-len(packet_lengths_in_order))))
    else:
...
"""
def interarrival_time(df: pd.DataFrame):
    time_deltas = np.array(df['time_delta'])
    
    if len(time_deltas) >= 40:
        return list(time_deltas[:K])
    else:
        temp = np.pad(time_deltas, (0, 40-len(time_deltas)), mode='edge')
        return list(temp)
